...
(function(){
  ...
  /* ---------- the marks (memory-board glyphs) — six, introduced as the night deepens ---------- */
  var GLYPH = [
    ...
    '<svg viewBox="0 0 40 40"><path d="M20 6 L33 20 L20 34 L7 20 Z"/></svg>',
    '<svg viewBox="0 0 40 40"><path d="M9 9 L31 31 M31 9 L9 31"/></svg>'
  ];
  ...
  /* ---------- day-phase visions ---------- */
  var VISIONS = [
    { id:0, title:"The Treeline", tiles:4, bg:"assets/kids.jpg", focal:"center 44%", scrim:"heavy",
    ...
    { id:1, title:"The Bark", tiles:5, bg:"assets/tabitha.jpg", focal:"center 28%", scrim:"med",
      lead:"A child lifts a small hand to the carved tree and adds a mark you haven\u2019t seen before \u2014 a diamond \u2014 then traces the symbol, stroke by stroke.",
      seq:[1,4,0,2],
      shard:{tag:"II \u00b7 The Carving", text:"A new mark joins the others. The order runs: triangle, diamond, first mark, ring. Longer now \u2014 and one more sign to hold."} },
    { id:2, title:"The Doorway", tiles:6, bg:"assets/doorway.jpg", focal:"center 42%", scrim:"med",
      lead:"In a lit doorway a child hums a tune with no words, and a last mark surfaces with it \u2014 a cross. Six marks in all now. Remember the whole of it.",
      seq:[2,5,3,1,4],
      shard:{tag:"III \u00b7 The Tune", text:"Six marks, five notes: ring, cross, broken line, triangle, diamond. The fullest pattern \u2014 and the way through the night."} }
  ];

  /* ---------- night rounds ---------- */
  var NIGHT = [
    { tiles:4, len:4, type:"plain", bg:"assets/kids.jpg",    focal:"center 44%" },
    { tiles:5, len:6, type:"decoy", bg:"assets/doorway.jpg", focal:"center 42%" },
    { tiles:6, len:8, type:"plain", bg:"assets/tower.jpg",   focal:"center 40%" }
  ];
  ...
  function boardHTML(N){
    N=N||4; var cols=(N<=4?2:3), t='';
    for(var i=0;i<N;i++){ t+='<button class="mtile" data-i="'+i+'" aria-label="mark '+(i+1)+'">'+GLYPH[i]+'</button>'; }
    return '<div class="mboard locked" id="mboard" style="grid-template-columns:repeat('+cols+',1fr);max-width:'+(cols===2?'340':'400')+'px">'+t+'</div>';
  }
  function bindBoard(){
  ...
  function genSeq(len, N){ N=N||4; var a=[],prev=-1,v; for(var i=0;i<len;i++){ do{ v=Math.floor(Math.random()*N); }while(v===prev); a.push(v); prev=v; } return a; }
  ...
  function nightIntro(){
    ...
    E.render(memHud(0)
      ...
      +'<p>Three will come, each longer than the last \u2014 and carrying marks she only learned tonight. Hold each one and a memory stays lit. Lose it, and a piece of her goes dark.</p>'
  ...
  function nightRound(idx){
    ...
    var seq=genSeq(R.len, R.tiles), altered=null;
    if(R.type==="decoy"){
      altered=seq.slice();
      var p=Math.floor(Math.random()*altered.length), v;
      do{ v=Math.floor(Math.random()*R.tiles); }while(v===altered[p]);
      altered[p]=v;
    }
    E.render(memHud(S.correct)
      +'<div class="scene fade">'
      +'<div class="q-progress">Night — pattern '+(idx+1)+' of 3 \u00b7 '+R.len+' marks'+(R.type==="decoy"?' \u00b7 one is a lie':'')+'</div>'
